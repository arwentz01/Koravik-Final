...
final class BeaconController
{
    ...
    private function index(): void
    {
        ...
        $links='';foreach($data['links'] as $l){$host=(string)($l['hostname']?:'krvk.nl');$url='https://'.$host.'/'.(string)$l['slug'];$links.='<article class="surface"><h3>'.self::e((string)$l['label']).'</h3><p><a href="'.self::e($url).'">'.self::e($url).'</a></p><small>'.(int)$l['visit_count'].' visits</small></article>';}
    ...
    private function createPage(): void{$this->csrf();$a=Security::requireAccount();try{(new BeaconService($this->database))->createPage((string)$a['id'],(string)($_POST['type']??''),(string)($_POST['title']??''),(string)($_POST['summary']??''),['value'=>(string)($_POST['value']??'')]);$_SESSION['flash']='Beacon page created.';}catch(RuntimeException $e){$_SESSION['flash']=$e->getMessage();}$this->go('/beacon');}
    ...
    private function publicPage(string $key): void{$p=(new BeaconService($this->database))->page($key);if(!$p){http_response_code(404);echo $this->page('Page not found','<section class="empty-state"><h1>This Beacon page is unavailable.</h1></section>');return;}$payload=$p['payload'];$value=self::e((string)($payload['value']??''));$body='<section class="surface public-beacon"><p class="eyebrow">'.self::e(ucwords(str_replace('_',' ',(string)$p['page_type']))).'</p><h1>'.self::e((string)$p['title']).'</h1><p>'.self::e((string)($p['summary']??'')).'</p>'.($value!==''?'<p class="prominent-value">'.$value.'</p>':'').'</section>';echo $this->page((string)$p['title'],$body);}
