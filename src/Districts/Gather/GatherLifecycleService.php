...
final class GatherLifecycleService
{
    ...
    private function ownedEvent(string $accountId,string $eventId): array { $s=$this->database->pdo()->prepare('SELECT * FROM gather_events WHERE id=:id AND account_id=:account LIMIT 1');$s->execute(['id'=>$eventId,'account'=>$accountId]);$event=$s->fetch();if(!$event)throw new RuntimeException('Event not found.');return $event; }
