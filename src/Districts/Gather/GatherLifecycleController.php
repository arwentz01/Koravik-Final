...
final class GatherLifecycleController
{
    ...
    private function agenda(string $eventId): void
    {
        ...
        $this->page((string)$event['title'].' agenda','<section class="page-heading"><div><p class="eyebrow">Event agenda</p><h1>'.self::e((string)$event['title']).'</h1><p>Build a personal plan and choose only the reminders you want.</p></div><a href="/gather/events/'.self::e($eventId).'">Event details</a></section>'.$ownerForm.'<section class="grid">'.($items?:'<article class="empty-state"><h2>No agenda items yet.</h2></article>').'</section>');
    }
