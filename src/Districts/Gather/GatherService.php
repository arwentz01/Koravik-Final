...
final class GatherService
{
    ...
    {
        $pdo=$this->database->pdo();$s=$pdo->prepare('SELECT * FROM gather_events WHERE id=:id AND status="published" AND visibility IN ("unlisted","public")');$s->execute(['id'=>$id]);$event=$s->fetch();return $event?$this->hydrateEvent($pdo,$event):null;
    }
    ...
    public function createEvent(string $accountId,array $input): string
    {
        ...
        $this->database->pdo()->prepare('INSERT INTO gather_events (id,account_id,title,description,venue,starts_at,ends_at,timezone,visibility,status,capacity,guest_registration_enabled,additional_guests_enabled,max_additional_guests,waitlist_enabled,automatic_waitlist_promotion,created_at,updated_at) VALUES (:id,:account_id,:title,:description,:venue,:starts_at,:ends_at,:timezone,:visibility,"published",:capacity,:guest_registration,:additional,:max_additional,:waitlist,:auto_promote,UTC_TIMESTAMP(),UTC_TIMESTAMP())')->execute([
            'id'=>$id,'account_id'=>$accountId,'title'=>$title,'description'=>(string)($input['description']??''),'venue'=>(string)($input['venue']??''),'starts_at'=>$starts,'ends_at'=>($input['ends_at']??'')!==''?str_replace('T',' ',(string)$input['ends_at']):null,'timezone'=>(string)($input['timezone']??'UTC'),'visibility'=>$visibility,'capacity'=>($input['capacity']??'')!==''?(int)$input['capacity']:null,'guest_registration'=>isset($input['guest_registration_enabled'])?1:0,'additional'=>$additional,'max_additional'=>$maxAdditional,'waitlist'=>isset($input['waitlist_enabled'])?1:0,'auto_promote'=>isset($input['automatic_waitlist_promotion'])?1:0
        ]);
    ...
    public function addSlot(string $accountId,string $eventId,string $type,string $title,int $quantity,array $rules=[]): void
    {
        ...
        $this->database->pdo()->prepare('INSERT INTO gather_signup_slots (id,event_id,slot_type,title,starts_at,ends_at,quantity_needed,multiple_signups_allowed,max_signups_per_participant,waitlist_enabled,overlapping_shifts_allowed,created_at,updated_at) VALUES (:id,:event_id,:type,:title,:starts_at,:ends_at,:quantity,:multiple,:maximum,:waitlist,:overlap,UTC_TIMESTAMP(),UTC_TIMESTAMP())')->execute(['id'=>self::uuid(),'event_id'=>$eventId,'type'=>$type,'title'=>$title,'starts_at'=>($rules['starts_at']??'')?:null,'ends_at'=>($rules['ends_at']??'')?:null,'quantity'=>max(1,$quantity),'multiple'=>isset($rules['multiple_signups_allowed'])?1:0,'maximum'=>($rules['max_signups_per_participant']??'')!==''?max(1,(int)$rules['max_signups_per_participant']):null,'waitlist'=>isset($rules['waitlist_enabled'])?1:0,'overlap'=>isset($rules['overlapping_shifts_allowed'])?1:0]);
    }
    ...
    private function hydrateEvent(PDO $pdo,array $event): array{$event['rsvps']=$this->rowsBy($pdo,'SELECT * FROM gather_rsvps WHERE event_id=:id ORDER BY created_at DESC',$event['id']);$event['slots']=$this->rowsBy($pdo,'SELECT * FROM gather_signup_slots WHERE event_id=:id ORDER BY slot_type,title',$event['id']);foreach($event['slots'] as &$slot){$slot['commitments']=$this->rowsBy($pdo,'SELECT * FROM gather_signup_commitments WHERE slot_id=:id AND status IN ("active","waitlist") ORDER BY status,waitlist_position,created_at',$slot['id']);}return $event;}
