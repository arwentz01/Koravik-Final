...
final class GatherCommandService
{
    ...
    public function dashboard(string $ownerId,string $eventId):array
    {
        ...
        $event['rsvps']=$this->rows($pdo,'SELECT r.*,c.party_count AS checked_in_party_count,c.checked_in_at,c.source AS checkin_source FROM gather_rsvps r LEFT JOIN gather_checkins c ON c.event_id=r.event_id AND c.rsvp_id=r.id AND c.corrected_at IS NULL WHERE r.event_id=:id ORDER BY FIELD(r.response,"yes","maybe","waitlist","no"),r.created_at',$eventId);
        ...
        $event['announcements']=$this->rows($pdo,'SELECT audience,title,message,urgency,email_enabled,created_at FROM gather_announcements WHERE event_id=:id ORDER BY created_at DESC LIMIT 20',$eventId);
        return $event;
    }

    public function updateSettings(string $ownerId,string $eventId,array $input):void
    {
        $this->ownedEvent($ownerId,$eventId);
        $visibility=in_array(($input['visibility']??''),['unlisted','public','restricted'],true)?$input['visibility']:'unlisted';
        $additional=isset($input['additional_guests_enabled']);
        $this->database->pdo()->prepare('UPDATE gather_events SET visibility=:visibility,capacity=:capacity,guest_registration_enabled=:guest,additional_guests_enabled=:additional,max_additional_guests=:max_guests,waitlist_enabled=:waitlist,automatic_waitlist_promotion=:auto,max_signups_per_participant=:max_signups,waitlist_offer_minutes=:offer,organizer_reply_to_email=:reply,updated_at=UTC_TIMESTAMP() WHERE id=:id')->execute([
            ...
            'guest'=>isset($input['guest_registration_enabled'])?1:0,
            'additional'=>$additional?1:0,
            'max_guests'=>$additional?max(0,(int)($input['max_additional_guests']??0)):0,
            'waitlist'=>isset($input['waitlist_enabled'])?1:0,
            'auto'=>isset($input['automatic_waitlist_promotion'])?1:0,
            'max_signups'=>($input['max_signups_per_participant']??'')!==''?max(1,(int)$input['max_signups_per_participant']):null,
            'offer'=>max(15,(int)($input['waitlist_offer_minutes']??1440)),
            'reply'=>filter_var(($input['organizer_reply_to_email']??''),FILTER_VALIDATE_EMAIL)?:null,
            'id'=>$eventId,
        ]);
    }
