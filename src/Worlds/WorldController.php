...
final class WorldController
{
    ...
    private function catalog(array $worlds): void
    {
        ...
        $body = $this->flashHtml() . '<section class="page-heading"><div><p class="eyebrow">Worlds</p><h1>Stories that respond with permission.</h1><p>Worlds are optional. Review what each World receives before installing or changing access.</p></div></section><div class="grid">' . ($cards ?: '<article class="empty-state"><h2>No Worlds are available.</h2></article>') . '</div>';
    ...
    private function detail(?array $world): void
    {
        ...
        $body = $this->flashHtml() . '<section class="panel world-detail"><p class="eyebrow">World</p><h1>' . self::escape((string) $world['name']) . '</h1><p class="lead">' . self::escape((string) $world['tagline']) . '</p><p>' . self::escape((string) $world['description']) . '</p><h2>Content notice</h2><p>' . self::escape((string) $world['content_notice']) . '</p><h2>Accessibility</h2><p>' . self::escape((string) $world['accessibility_notes']) . '</p><p class="meta">Package ' . self::escape((string) $world['package_version']) . ' · Status: ' . self::escape(ucfirst($status)) . '</p>' . $action . '</section>' . $permission;
