...
final class ChapterTwoController
{
    public function handle(string $method,string $path): bool
    {
        ...
        try {
            ...
            if($method==='POST'&&$path==='/worlds/epic-ordinary/play/chapter-three/begin'){$this->csrf();$service->beginChapterThree($accountId);$_SESSION['flash']='The sound behind the library wall is clearer now.';$this->redirect('/worlds/epic-ordinary/play');}
            if($method==='POST'&&$path==='/worlds/epic-ordinary/play/chapter-three/choose'){$this->csrf();$service->chooseListeningTruth($accountId,(string)($_POST['choice']??''));$_SESSION['flash']='The house will keep the truth you chose.';$this->redirect('/worlds/epic-ordinary/play');}
    ...
    private function home(array $state): void
    {
        ...
        if(!$state['support_choice']){
            ...
            $sceneHtml='<p class="eyebrow">Chapter Two · The Eastern Room</p><h1>The room has taken its first shape.</h1><blockquote>“'.self::e($choice).' It does not need to become everything today.”</blockquote><p>The choice is preserved as fictional World State. It does not create or alter a real-life Quest.</p><div class="local-actions"><a class="button" href="/home/rooms/eastern_room">Visit the Eastern Room</a><a class="button secondary" href="/worlds/epic-ordinary/progress">View World State</a><a class="button secondary" href="/hearth">Return to Hearth</a></div><section class="chapter-three-threshold"><p class="eyebrow">Chapter Three · The Listening Wall</p><h2>A sound has returned behind the library plaster.</h2><p>The Caretaker has heard it too, but will not name it for you.</p><form method="post" action="/worlds/epic-ordinary/play/chapter-three/begin">'.$this->csrfField().'<button class="button" type="submit">Listen with the Caretaker</button></form></section>';
        } elseif($scene==='the-sound-behind-plaster') {
            $sceneHtml='<p class="eyebrow">Chapter Three · The Listening Wall</p><h1>Something quiet is asking to be remembered.</h1><blockquote>“A house can keep an echo without making it an order. Which truth should live here?”</blockquote><form method="post" action="/worlds/epic-ordinary/play/chapter-three/choose">'.$this->csrfField().'<div class="choice-list listening-choice-list" aria-label="Choose what the Listening Wall should keep"><button name="choice" value="courage"><strong>Keep the courage it took to begin</strong><span>Let imperfect beginnings remain worthy of notice.</span></button><button name="choice" value="care"><strong>Keep the care that made room</strong><span>Remember the gentleness behind what was restored.</span></button><button name="choice" value="possibility"><strong>Keep what is still possible</strong><span>Leave one honest opening for what has not been written.</span></button></div></form><section class="world-boundary-note"><h2>What this choice changes</h2><p>It records one fictional truth, completes one World objective, creates one keepsake, changes Caretaker trust with a visible reason, and adds one source-labeled echo to the Healing Home Library. It does not inspect or change Quests, Chronicle, Companion memory, or private notes.</p></section>';
        } else {
            $choice=self::e((string)($state['listening_choice']['choice_label']??'What remains possible'));
            $sceneHtml='<p class="eyebrow">Chapter Three · The Listening Wall</p><h1>The wall answers in your own words.</h1><blockquote>“'.$choice.'. The house can hold that without asking you to prove it again.”</blockquote><p>This is durable fictional World State. Healing Home may display its source-labeled echo, but no real-life record was created.</p><div class="local-actions"><a class="button" href="/home/rooms/library">Hear the Library echo</a><a class="button secondary" href="/worlds/epic-ordinary/progress">Review what changed</a><a class="button secondary" href="/hearth">Return to Hearth</a></div>';
        }
