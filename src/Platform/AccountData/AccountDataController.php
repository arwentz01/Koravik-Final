...
final class AccountDataController
{
    ...
    public function handle(string $method,string $path): bool
    {
        ...
        if($method==='GET'&&$path==='/settings/data'){$this->index($a,$s->review($a));return true;}
    ...
    private function index(string $a,array $review): void
    {
        ...
        $counts='';foreach($review['counts'] as $label=>$count)$counts.='<li>'.self::e((string)$label).': '.(int)$count.' account-owned record(s)</li>';$excluded='';foreach($review['excluded'] as $x)$excluded.='<li>'.self::e((string)$x).'</li>';
        echo '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Data controls · Koravik</title><link rel="stylesheet" href="/assets/app.css"></head><body><main class="page">'.$this->flash().'<section class="page-heading"><div><p class="eyebrow">Settings · Data controls</p><h1>Export or close your account.</h1><p>Exports exclude credentials and secrets. Closure is staged, cancellable, and owner-specific.</p></div><a href="/settings">Back to Settings</a></section><section class="settings-card account-data-export-review"><h2>Account Data Export Review</h2><p>Review what each source owner contributes before generating an export.</p><ul>'.$counts.'</ul><h3>Excluded from exports</h3><ul>'.$excluded.'</ul></section><section class="settings-card"><h2>Account export</h2><form method="post" action="/settings/data/exports"><input type="hidden" name="csrf" value="'.self::e(Security::csrfToken()).'"><select name="format"><option value="json">Structured JSON</option><option value="html">Human-readable export manifest</option></select><button class="button">Create export</button></form><ul>'.$exports.'</ul></section><section class="settings-card account-closure-consequence-preview"><h2>Account Closure Consequence Preview</h2><p>Closure keeps a seven-day cancellation window. Afterward, source modules delete or anonymize account-owned data and retain only minimal closure/audit ledger evidence.</p></section>'.$closureHtml.'</main></body></html>';
    }
