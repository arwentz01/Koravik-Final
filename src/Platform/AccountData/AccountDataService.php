...
{
    private const OWNERS=['Hearth','Quests','Chronicle','Companion','Worlds','Platform'];
    ...
    {
        $pdo=$this->database->pdo();$roomNoteColumns=$this->roomNotesAvailable($pdo)?'note_text,note_updated_at':'NULL AS note_text,NULL AS note_updated_at';$tables=['account'=>'SELECT id,email,display_name,role,status,created_at,updated_at FROM platform_accounts WHERE id=:account_id','settings'=>'SELECT * FROM account_settings WHERE account_id=:account_id','healing_home_state'=>'SELECT account_id,atmosphere,current_room,last_returned_at,created_at,updated_at FROM healing_home_state WHERE account_id=:account_id','healing_home_rooms'=>'SELECT room_key,name,state,'.$roomNoteColumns.',sort_order,unlocked_at,created_at,updated_at FROM healing_home_rooms WHERE account_id=:account_id ORDER BY sort_order,name','relationship_conversations'=>'SELECT character_key,conversation_type,prompt_key,player_choice,character_response,remembered_context,created_at FROM relationship_conversations WHERE account_id=:account_id ORDER BY created_at','hearth_daily_focus'=>'SELECT id,focus_date,intention,created_at,updated_at FROM hearth_daily_focus WHERE account_id=:account_id','hearth_daily_focus_entries'=>'SELECT e.focus_id,e.quest_occurrence_id,e.position,e.created_at FROM hearth_daily_focus_entries e JOIN hearth_daily_focus f ON f.id=e.focus_id WHERE f.account_id=:account_id','quests'=>'SELECT * FROM quests WHERE account_id=:account_id','occurrences'=>'SELECT * FROM quest_occurrences WHERE account_id=:account_id','chronicle'=>'SELECT * FROM chronicle_entries WHERE account_id=:account_id AND deleted_at IS NULL','companion_proposals'=>'SELECT id,proposal_type,status,version,title,reasoning,source_context,owning_module,consequence,created_at,updated_at FROM companion_proposals WHERE account_id=:account_id','companion_memories'=>'SELECT * FROM companion_memories WHERE account_id=:account_id AND status<>"deleted"','world_installations'=>'SELECT * FROM world_installations WHERE account_id=:account_id','world_reaction_reviews'=>'SELECT reaction_id,reviewed_at FROM world_reaction_reviews WHERE account_id=:account_id','notifications'=>'SELECT * FROM notifications WHERE account_id=:account_id','audit'=>'SELECT action,subject_type,subject_id,occurred_at FROM audit_log WHERE account_id=:account_id ORDER BY occurred_at'];$data=[];
    ...
    private function processClosure(string $closureId,string $accountId): void
    {
        $this->database->transaction(function(PDO $pdo) use($closureId,$accountId): void {$pdo->prepare('UPDATE account_closures SET status="processing",processing_started_at=UTC_TIMESTAMP() WHERE id=:id')->execute(['id'=>$closureId]);
            $pdo->prepare('DELETE FROM hearth_daily_focus WHERE account_id=:account_id')->execute(['account_id'=>$accountId]);$pdo->prepare('DELETE FROM relationship_conversations WHERE account_id=:account_id')->execute(['account_id'=>$accountId]);$pdo->prepare('DELETE FROM healing_home_keepsake_placements WHERE account_id=:account_id')->execute(['account_id'=>$accountId]);$pdo->prepare('DELETE FROM healing_home_keepsakes WHERE account_id=:account_id')->execute(['account_id'=>$accountId]);$pdo->prepare('DELETE FROM healing_home_changes WHERE account_id=:account_id')->execute(['account_id'=>$accountId]);$pdo->prepare('DELETE FROM healing_home_rooms WHERE account_id=:account_id')->execute(['account_id'=>$accountId]);$pdo->prepare('DELETE FROM healing_home_state WHERE account_id=:account_id')->execute(['account_id'=>$accountId]);$this->step($pdo,$closureId,'Hearth','deleted personal Daily Focus and Healing Home composition');
    ...
    private function roomNotesAvailable(PDO $pdo): bool {$s=$pdo->prepare('SELECT COUNT(*) FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name="healing_home_rooms" AND column_name IN ("note_text","note_updated_at")');$s->execute();return (int)$s->fetchColumn()===2;}
