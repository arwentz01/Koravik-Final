...
final class HearthLayoutController
{
    ...
    private function render(array $layout): void
    {
        ...
        $previewItems='';foreach($layout as $row)$previewItems.='<li>'.self::e(HearthLayoutService::WIDGETS[(string)$row['widget_key']]).' · '.((bool)$row['visible']?'shown':'hidden').'</li>';
        $preview='<section class="panel hearth-layout-preview"><h2>Layout preview</h2><p>The greeting and “What matters now” remain required. Optional sections will appear in this order and still link back to their source owners.</p><ol>'.$previewItems.'</ol></section>';
        $body=($flash!==''?'<div class="notice" role="status">'.self::e($flash).'</div>':'').'<section class="page-heading hearth-customization-completion"><div><p class="eyebrow">Hearth customization</p><h1>What should appear on your Hearth?</h1><p>Keep the center useful, bounded, and yours. Reordering changes presentation only; it never edits Quests, Chronicle, Worlds, Households, Organizations, or trust controls.</p></div><a class="button secondary" href="/hearth">Preview on Hearth</a></section><form method="post" action="/hearth/customize"><input type="hidden" name="csrf" value="'.self::e(Security::csrfToken()).'"><ol class="preference-list">'.$rows.'</ol><div class="form-actions"><button class="button" type="submit">Save layout</button></div></form><form method="post" action="/hearth/customize/reset"><input type="hidden" name="csrf" value="'.self::e(Security::csrfToken()).'"><button class="button secondary" type="submit">Restore defaults</button></form>'.$preview;
