...
final class PrivacyController
{
    ...
    private function privacy(PrivacyService $service,string $accountId): void
    {
        ...
        foreach($service->grants($accountId) as $grant) {
            ...
            $control=!empty($grant['readonly'])
                ? '<p class="meta">Managed at the source surface. This center shows the current boundary.</p>'
                : '<form method="post" action="/privacy/'.self::e((string)$grant['installation_id']).'/'.self::e((string)$grant['fact_key']).'/'.($active?'revoke':'grant').'">'.$this->csrfField().'<button class="button '.($active?'secondary':'').'" type="submit">'.($active?'Revoke permission':'Grant permission').'</button></form>';
            $cards.='<article class="consent-card"><div><p class="eyebrow">'.self::e((string)$grant['source']).' → '.self::e((string)$grant['recipient']).'</p><h2>'.self::e((string)$grant['label']).'</h2><p>'.self::e((string)$grant['purpose']).'</p><dl><div><dt>Status</dt><dd>'.($active?'Granted':'Revoked').'</dd></div><div><dt>Recipient status</dt><dd>'.self::e((string)$grant['installation_status']).'</dd></div><div><dt>Last used</dt><dd>'.self::e((string)($grant['last_used_at']??'Never')).'</dd></div><div><dt>Revocation effect</dt><dd>Stops future delivery. Existing source records, World State, reactions, and audit history remain.</dd></div></dl></div>'.$control.'</article>';
        }
        $auditSurface='<section class="surface cross-module-privacy-audit-surface"><h2>Cross-Module Privacy Audit Surface</h2><p>Source owners stay visible: Quests owns actions, Chronicle owns saved prose, Gather owns event truth, Health owns private check-ins, Companion owns proposals, Worlds owns fictional interpretation, and Moment Engine owns presentation state only.</p><p>What never crossed modules: raw Health notes, Companion memory, guest contact lists, private Chronicle prose, account secrets, and unrelated records.</p></section>';
        $body=$this->flash().'<section class="page-heading"><div><p class="eyebrow">Privacy and consent</p><h1>What may Koravik use?</h1><p>Review each future fact grant, its purpose, recipient, last use, and the effect of revocation.</p></div><a class="button secondary" href="/audit">View audit activity</a></section>'.$auditSurface.'<div class="consent-list">'.($cards?:'<section class="empty-state"><h2>No active capability grants.</h2></section>').'</div>';
    ...
    private function audit(PrivacyService $service,string $accountId): void
    {
        ...
        foreach($service->audit($accountId) as $row) {
            $items.='<article class="audit-row"><div><p class="eyebrow">'.self::e((string)$row['module']).'</p><h2>'.self::e((string)$row['summary']).'</h2><p class="meta">Actor: your account · Approval context: source-owned workflow · '.self::e((string)$row['occurred_at']).' UTC</p></div><details open><summary>Inspectable audit detail</summary><p>Action: <code>'.self::e((string)$row['action']).'</code></p><p>Affected record: '.self::e((string)$row['subject_type']).' / '.self::e((string)$row['subject_id']).'</p><p>This historical record cannot be edited and does not replace the source module history.</p></details></article>';
        }
