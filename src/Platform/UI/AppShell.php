...
final class AppShell
{
    public function apply(string $html,?array $account,string $path): string
    {
        ...
        $accountLinks=$this->link('/guide','Koravik guide',$path).$this->link('/settings','Settings',$path).$this->link('/privacy','Privacy',$path).$this->link('/audit','Audit activity',$path).$this->link('/settings/security','Security',$path).$this->link('/settings/data','Data controls',$path);
        $header='<header class="app-shell-header"><a class="brand" href="/hearth">Koravik</a><button class="nav-toggle" type="button" aria-expanded="false" aria-controls="primary-navigation" onclick="const n=document.getElementById(\'primary-navigation\');const open=this.getAttribute(\'aria-expanded\')===\'true\';this.setAttribute(\'aria-expanded\',String(!open));n.hidden=open;">Menu</button><div id="primary-navigation" class="shell-navigation"><nav class="primary-nav" aria-label="Primary">'.$primaryHtml.'</nav><nav class="utility-nav" aria-label="Utilities">'.$utilities.'</nav><details class="account-menu"><summary>'.self::e((string)$account['display_name']).'</summary><div class="account-menu-panel">'.$accountLinks.'<form method="post" action="/logout"><input type="hidden" name="csrf" value="'.self::e(\Koravik\Platform\Security\Security::csrfToken()).'"><button type="submit">Sign out</button></form></div></details></div></header>';
    ...
    private function link(string $href,string $label,string $path,bool $escape=true):string{$active=$path===$href||($href!=='/hearth'&&str_starts_with($path,$href.'/'));return '<a href="'.$href.'"'.($active?' aria-current="page"':'').'>'.($escape?self::e($label):$label).'</a>';}
