...
final class AppShell
{
    public function apply(string $html,?array $account,string $path): string
    {
        ...
        $accountLinks=$this->link('/hearth','Hearth overview',$path).$this->link('/guide','Koravik guide',$path).$this->link('/settings','Settings',$path).$this->link('/settings/accessibility','Accessibility',$path).$this->link('/privacy','Privacy',$path).$this->link('/audit','Audit activity',$path).$this->link('/settings/security','Security',$path).$this->link('/settings/data','Data controls',$path);
