...
final class AppShell
{
    public function apply(string $html,?array $account,string $path): string
    {
        ...
        if(!str_contains($html,'app-shell.css'))$html=str_replace('</head>','<link rel="stylesheet" href="/assets/app-shell.css?v=brand-v1"></head>',$html);
        $primary=['/home'=>'Home','/journey'=>'Journey','/quests'=>'Quests','/gather'=>'Gather','/beacon'=>'Beacon','/chronicle'=>'Chronicle','/worlds'=>'Worlds','/companion'=>'Companion'];
        ...
        $accountLinks=$this->link('/hearth','Hearth overview',$path).$this->link('/guide','Koravik guide',$path).$this->link('/settings','Settings',$path).$this->link('/settings/accessibility','Accessibility',$path).$this->link('/recovery-center','Recovery center',$path).$this->link('/privacy','Privacy',$path).$this->link('/audit','Audit activity',$path).$this->link('/settings/security','Security',$path).$this->link('/settings/sessions','Signed-in devices',$path).$this->link('/settings/data','Data controls',$path);
        $header='<header class="app-shell-header mobile-navigation-pass"><a class="brand" href="/home">Koravik</a><button class="nav-toggle" type="button" aria-expanded="false" aria-controls="primary-navigation" onclick="const n=document.getElementById(\'primary-navigation\');const open=this.getAttribute(\'aria-expanded\')===\'true\';this.setAttribute(\'aria-expanded\',String(!open));n.hidden=open;">Menu</button><div id="primary-navigation" class="shell-navigation" hidden><nav class="primary-nav" aria-label="Primary">'.$primaryHtml.'</nav><nav class="utility-nav" aria-label="Utilities">'.$utilities.'</nav><details class="account-menu"><summary>'.self::e((string)$account['display_name']).'</summary><div class="account-menu-panel">'.$accountLinks.'<form method="post" action="/logout"><input type="hidden" name="csrf" value="'.self::e(\Koravik\Platform\Security\Security::csrfToken()).'"><button type="submit">Sign out</button></form></div></details></div></header>';
        $html=preg_replace('#<header class="app-header">.*?</header>#s',$header,$html,1)??$html;
        if(!str_contains($html,'app-shell-header')){$html=preg_replace('~<a class="skip-link" href="#main">.*?</a>~s','',$html,1)??$html;$html=preg_replace('#(<body[^>]*>)#','$1<a class="skip-link" href="#main">Skip to content</a>'.$header,$html,1)??$html;}
