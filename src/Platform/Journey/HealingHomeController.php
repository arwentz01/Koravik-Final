...
final class HealingHomeController
{
    ...
    private function renderHome(array $account, array $journey): string
    {
        ...
        $focusHtml = $focus
            ? '<article class="home-focus room-card room-quest-board" aria-labelledby="quest-board-title"><span class="room-glow" aria-hidden="true"></span><p class="eyebrow">Quest Board</p><h2 id="quest-board-title">' . self::e((string) $focus['title']) . '</h2><p>' . self::e((string) ($focus['purpose'] ?: 'A commitment you chose to carry.')) . '</p><div class="next-step"><span>Next meaningful step</span><strong>' . self::e((string) ($focus['next_step'] ?: 'Open the Quest and decide what comes next.')) . '</strong></div><a class="button" href="/quests/' . self::e((string) $focus['id']) . '">Continue Quest</a></article>'
            : '<article class="home-focus room-card room-quest-board empty-state" aria-labelledby="quest-board-title"><span class="room-glow" aria-hidden="true"></span><p class="eyebrow">Quest Board</p><h2 id="quest-board-title">Nothing is demanding your attention.</h2><p>You can begin something meaningful when you are ready.</p><a class="button" href="/quests/create">Begin a Quest</a></article>';

        $memory = $journey['chronicle'];
        $memoryHtml = $memory
            ? '<article class="home-place room-card room-journal-table" aria-labelledby="journal-table-title"><p class="eyebrow">Journal Table</p><h2 id="journal-table-title">' . self::e((string) $memory['title']) . '</h2><p>' . self::e((string) $memory['body']) . '</p><a href="/chronicle">Open Chronicle</a></article>'
            : '<article class="home-place room-card room-journal-table" aria-labelledby="journal-table-title"><p class="eyebrow">Journal Table</p><h2 id="journal-table-title">Your story has room to gather.</h2><p>Reflections and meaningful moments will rest here.</p><a href="/chronicle">Open Chronicle</a></article>';
        ...
        $relationshipHtml = '<article class="home-place room-card room-relationships companion-seat" aria-labelledby="relationship-title"><p class="eyebrow">Guest and Resident Memory</p><h2 id="relationship-title">People remember what was shared.</h2>' . ($relationships ? '<ul class="relationship-list">' . $relationships . '</ul>' : '<p>No one has gathered here yet.</p>') . '</article>';
