...
final class JourneyArcService
{
    ...
    public function decideInvitation(string $accountId, string $id, string $decision): ?string
    {
        ...
        return $this->database->transaction(function (PDO $pdo) use ($accountId, $id, $decision): ?string {
            $s = $pdo->prepare('SELECT * FROM story_invitations WHERE id=:id AND account_id=:account_id FOR UPDATE');
            $s->execute(['id'=>$id,'account_id'=>$accountId]);
            $row = $s->fetch();
            if (!$row || !in_array($row['status'], ['open','snoozed'], true)) throw new RuntimeException('That invitation is no longer waiting.');
            if ($decision === 'accept') {
                $questId = (new QuestService($this->database))->create($accountId, (string)$row['suggested_quest_title'], '', [
                    'quest_type'=>'journey','purpose'=>(string)($row['suggested_purpose']??''),'next_step'=>(string)($row['suggested_next_step']??''),
                    'origin_type'=>'story','origin_reference'=>(string)$row['world_key'].':'.(string)$row['invitation_key'],'frequency'=>'none','starts_on'=>gmdate('Y-m-d')
                ]);
                $pdo->prepare('UPDATE story_invitations SET status="accepted",accepted_quest_id=:quest_id,decided_at=UTC_TIMESTAMP(),updated_at=UTC_TIMESTAMP() WHERE id=:id')->execute(['quest_id'=>$questId,'id'=>$id]);
                return $questId;
            }
            if ($decision === 'snooze') {
                $pdo->prepare('UPDATE story_invitations SET status="snoozed",snoozed_until=DATE_ADD(UTC_TIMESTAMP(),INTERVAL 7 DAY),updated_at=UTC_TIMESTAMP() WHERE id=:id')->execute(['id'=>$id]);
                return null;
            }
            $pdo->prepare('UPDATE story_invitations SET status="declined",decided_at=UTC_TIMESTAMP(),updated_at=UTC_TIMESTAMP() WHERE id=:id')->execute(['id'=>$id]);
            return null;
        });
    }
    ...
    public function converse(string $accountId, string $choice): void
    {
        ...
        $last = $this->database->pdo()->prepare('SELECT memory_text FROM journey_relationship_memories WHERE account_id=:account_id AND character_key="caretaker" ORDER BY remembered_at DESC LIMIT 1');
        $last->execute(['account_id'=>$accountId]);
        $memory = $last->fetchColumn();
        $this->database->pdo()->prepare('INSERT INTO relationship_conversations (id,account_id,character_key,conversation_type,prompt_key,player_choice,character_response,remembered_context,created_at) VALUES (:id,:account_id,"caretaker","check_in","home_fireplace",:choice,:response,:memory,UTC_TIMESTAMP())')->execute([
            'id'=>self::uuid(),'account_id'=>$accountId,'choice'=>$choice,'response'=>$responses[$choice],'memory'=>$memory ?: null
        ]);
    }
    ...
    public function createSourceProposal(string $accountId, string $domain, string $reference, string $kind, string $title): void
    {
        ...
        $this->database->pdo()->prepare('INSERT IGNORE INTO quest_source_proposals (id,account_id,source_domain,source_reference,title,purpose,next_step,proposal_kind,status,created_at) VALUES (:id,:account_id,:domain,:reference,:title,:purpose,:next_step,:kind,"open",UTC_TIMESTAMP())')->execute([
            'id'=>self::uuid(),'account_id'=>$accountId,'domain'=>$domain,'reference'=>$reference,'title'=>$title,
            'purpose'=>$domain === 'beacon' ? 'Prepare for or follow through on an event that matters.' : 'Contribute to something meaningful with other people.',
            'next_step'=>$domain === 'beacon' ? 'Review the event and choose one preparation step.' : 'Clarify the contribution you are willing to make.','kind'=>$kind
        ]);
    }
    ...
    public function decideProposal(string $accountId, string $id, string $decision): ?string
    {
        ...
        return $this->database->transaction(function (PDO $pdo) use ($accountId,$id,$decision): ?string {
            $s=$pdo->prepare('SELECT * FROM quest_source_proposals WHERE id=:id AND account_id=:account_id AND status="open" FOR UPDATE');
            $s->execute(['id'=>$id,'account_id'=>$accountId]); $row=$s->fetch();
            if(!$row) throw new RuntimeException('That proposal is no longer waiting.');
            if($decision==='decline'){$pdo->prepare('UPDATE quest_source_proposals SET status="declined",decided_at=UTC_TIMESTAMP() WHERE id=:id')->execute(['id'=>$id]);return null;}
            $questId=(new QuestService($this->database))->create($accountId,(string)$row['title'],'',[
                'quest_type'=>'action','purpose'=>(string)($row['purpose']??''),'next_step'=>(string)($row['next_step']??''),'origin_type'=>(string)$row['source_domain'],'origin_reference'=>(string)$row['source_reference'],'frequency'=>'none','starts_on'=>gmdate('Y-m-d')
            ]);
            $pdo->prepare('UPDATE quest_source_proposals SET status="accepted",created_quest_id=:quest_id,decided_at=UTC_TIMESTAMP() WHERE id=:id')->execute(['quest_id'=>$questId,'id'=>$id]);
            return $questId;
        });
    }
