...
final class ReturnService
{
    ...
    public function observe(string $accountId): bool
    {
        return $this->database->transaction(function (PDO $pdo) use ($accountId): bool {
            ...
            if(!(bool)$activity['return_pending'] && $activity['last_seen_at'] && strtotime($now)-strtotime((string)$activity['last_seen_at'])>=7*86400) {
                ...
                $pdo->prepare('INSERT INTO platform_outbox (id,event_name,event_version,account_id,payload_json,status,attempts,available_at,occurred_at,created_at) VALUES (:id,"Platform.PlayerReturned",1,:account_id,:payload,"pending",0,:available_at,:occurred_at,:created_at)')->execute(['id'=>self::uuid(),'account_id'=>$accountId,'payload'=>json_encode(['days_away'=>$daysAway],JSON_THROW_ON_ERROR),'available_at'=>$now,'occurred_at'=>$now,'created_at'=>$now]);
    ...
    public function summary(string $accountId): array
    {
        ...
        $stale=$pdo->prepare($base.' AND q.lifecycle_status="active" AND qo.status IN ("available","scheduled") AND qo.scheduled_for<DATE_SUB(CURRENT_DATE(),INTERVAL 3 DAY) ORDER BY qo.scheduled_for ASC LIMIT 12');$stale->execute(['account_id'=>$accountId]);
        $relevant=$pdo->prepare($base.' AND q.lifecycle_status="active" AND qo.status IN ("available","scheduled") AND qo.scheduled_for BETWEEN DATE_SUB(CURRENT_DATE(),INTERVAL 3 DAY) AND CURRENT_DATE() ORDER BY qo.scheduled_for ASC LIMIT 6');$relevant->execute(['account_id'=>$accountId]);
    ...
    public function decide(string $accountId,string $occurrenceId,string $action,?string $newDate=null): void
    {
        ...
        $this->database->transaction(function(PDO $pdo) use($accountId,$occurrenceId,$action,$newDate): void {
            ...
            if($action==='resume') {
                $pdo->prepare('UPDATE quest_occurrences SET status="available",available_at=:available_at,updated_at=:updated_at WHERE id=:id')->execute(['available_at'=>$now,'updated_at'=>$now,'id'=>$occurrenceId]);$eventName='Quests.QuestOccurrenceResumed';
            } elseif($action==='skip') {
                $pdo->prepare('UPDATE quest_occurrences SET status="skipped",skipped_at=:skipped_at,updated_at=:updated_at WHERE id=:id')->execute(['skipped_at'=>$now,'updated_at'=>$now,'id'=>$occurrenceId]);$eventName='Quests.QuestOccurrenceSkipped';
            } elseif($action==='dismiss') {
                $pdo->prepare('UPDATE quest_occurrences SET status="dismissed",dismissed_at=:dismissed_at,updated_at=:updated_at WHERE id=:id')->execute(['dismissed_at'=>$now,'updated_at'=>$now,'id'=>$occurrenceId]);$eventName='Quests.QuestOccurrenceDismissed';
            } else {
                $newDate=trim((string)$newDate);
                if(!preg_match('/^\d{4}-\d{2}-\d{2}$/',$newDate)) throw new RuntimeException('Choose a valid new date.');
                $pdo->prepare('UPDATE quest_occurrences SET rescheduled_from=scheduled_for,scheduled_for=:scheduled_for,status="scheduled",available_at=:available_at,updated_at=:updated_at WHERE id=:id')->execute(['scheduled_for'=>$newDate,'available_at'=>$now,'updated_at'=>$now,'id'=>$occurrenceId]);$eventName='Quests.QuestOccurrenceRescheduled';$payload['scheduled_date']=$newDate;
            }
            $pdo->prepare('INSERT INTO platform_outbox (id,event_name,event_version,account_id,payload_json,status,attempts,available_at,occurred_at,created_at) VALUES (:id,:event_name,1,:account_id,:payload,"pending",0,:available_at,:occurred_at,:created_at)')->execute(['id'=>self::uuid(),'event_name'=>$eventName,'account_id'=>$accountId,'payload'=>json_encode($payload,JSON_THROW_ON_ERROR),'available_at'=>$now,'occurred_at'=>$now,'created_at'=>$now]);
