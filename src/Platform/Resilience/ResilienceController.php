...
final class ResilienceController
{
    ...
    {
        $items='';foreach($service->drafts($accountId) as $draft){$payload=(array)$draft['payload'];$summary=implode(', ',array_map('strval',array_slice($payload,0,3)));$resume=str_starts_with((string)$draft['form_key'],'source_review.')?'<a class="button secondary" href="/source-review/drafts/'.self::e((string)$draft['id']).'/resume">Resume Source Review draft</a>':'';$items.='<article class="settings-card"><p class="eyebrow">'.self::e((string)$draft['form_key']).'</p><h2>Saved '.self::e((string)$draft['updated_at']).' UTC</h2><p>'.self::e($summary?:'Empty draft').'</p><p class="meta">Expires '.self::e((string)$draft['expires_at']).' UTC</p><p>'.$resume.'</p><form method="post" action="/recovery-center/drafts/'.self::e((string)$draft['id']).'/delete">'.$this->csrfField().'<button class="quiet-button">Remove draft</button></form></article>';}
