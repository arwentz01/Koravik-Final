...
final class SettingsController
{
    ...
    private function render(array $s): void
    {
        ...
        $body=($flash!==''?'<div class="notice" role="status">'.self::e($flash).'</div>':'').'<section class="page-heading"><div><p class="eyebrow">Settings</p><h1>How should Koravik work for you?</h1><p>Low-risk preferences live here. Privacy and data controls remain explicit.</p></div></section><form class="settings-sections" method="post" action="/settings"><input type="hidden" name="csrf" value="'.self::e(Security::csrfToken()).'"><section class="settings-card"><h2>Account</h2><label>Display name<input name="display_name" maxlength="100" required value="'.self::e((string)$s['display_name']).'"></label></section><section class="settings-card"><h2>Appearance and accessibility</h2><label>Appearance<select name="appearance">'.$option('system',(string)$s['appearance'],'Use device setting').$option('light',(string)$s['appearance'],'Light').$option('dark',(string)$s['appearance'],'Dark').'</select></label><label class="check-row"><input type="checkbox" name="reduced_motion"'.((bool)$s['reduced_motion']?' checked':'').'> Reduce nonessential motion</label><label class="check-row"><input type="checkbox" name="high_contrast"'.((bool)$s['high_contrast']?' checked':'').'> Increase interface contrast</label></section><section class="settings-card"><h2>Time and dates</h2><label>Time zone<select name="timezone">'.$option('America/New_York',(string)$s['timezone'],'Eastern Time').$option('America/Chicago',(string)$s['timezone'],'Central Time').$option('America/Denver',(string)$s['timezone'],'Mountain Time').$option('America/Los_Angeles',(string)$s['timezone'],'Pacific Time').$option('UTC',(string)$s['timezone'],'UTC').'</select></label><label>Date format<select name="date_format">'.$option('month_day_year',(string)$s['date_format'],'Month / day / year').$option('year_month_day',(string)$s['date_format'],'Year / month / day').$option('day_month_year',(string)$s['date_format'],'Day / month / year').'</select></label></section><section class="settings-card"><h2>Attention and privacy</h2><p><a href="/notifications/preferences">Notification preferences</a></p><p><a href="/privacy">Privacy and consent</a></p><p><a href="/audit">Audit activity</a></p></section><section class="settings-card"><h2>Data controls</h2><p>Your District records remain owned by their source modules. World permissions can be revoked without deleting source history.</p><p>Account export and deletion execution are not yet available; Koravik does not pretend otherwise.</p></section><button class="button" type="submit">Save settings</button></form>';
