...
final class NotificationService
{
    ...
    public function create(string $accountId,string $sourceModule,string $category,string $title,string $body,string $targetUrl,string $reason,?string $sourceEventId): void
    {
        ...
        $this->database->pdo()->prepare('INSERT IGNORE INTO notifications (id,account_id,source_module,category,title,body,target_url,reason,source_event_id,created_at) VALUES (:id,:account_id,:source_module,:category,:title,:body,:target_url,:reason,:source_event_id,UTC_TIMESTAMP())')->execute([
            'id'=>self::uuid(),'account_id'=>$accountId,'source_module'=>$sourceModule,'category'=>$category,'title'=>$title,'body'=>$body,'target_url'=>$targetUrl,'reason'=>$reason,'source_event_id'=>$sourceEventId,
        ]);
    }
    ...
    {
        $sql=match($action){
            'read'=>'UPDATE notifications SET read_at=COALESCE(read_at,UTC_TIMESTAMP()) WHERE id=:id AND account_id=:account_id AND dismissed_at IS NULL',
            'unread'=>'UPDATE notifications SET read_at=NULL WHERE id=:id AND account_id=:account_id AND dismissed_at IS NULL',
            'dismiss'=>'UPDATE notifications SET dismissed_at=UTC_TIMESTAMP() WHERE id=:id AND account_id=:account_id',
            default=>throw new RuntimeException('Unknown notification action.'),
        };
        $statement=$this->database->pdo()->prepare($sql);
        $statement->execute(['id'=>$notificationId,'account_id'=>$accountId]);
    ...
    {
        $this->ensurePreferences($accountId);
        $statement=$this->database->pdo()->prepare('SELECT category,enabled FROM notification_preferences WHERE account_id=:account_id');
        $statement->execute(['account_id'=>$accountId]);
        $values=[];
    ...
    {
        $this->database->transaction(function(PDO $pdo) use($accountId,$enabledCategories): void {
            $statement=$pdo->prepare('INSERT INTO notification_preferences (account_id,category,enabled,updated_at) VALUES (:account_id,:category,:enabled,UTC_TIMESTAMP()) ON DUPLICATE KEY UPDATE enabled=VALUES(enabled),updated_at=VALUES(updated_at)');
            foreach(array_keys(self::CATEGORIES) as $category) $statement->execute(['account_id'=>$accountId,'category'=>$category,'enabled'=>in_array($category,$enabledCategories,true)?1:0]);
        });
    }
