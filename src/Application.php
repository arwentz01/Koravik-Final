...
final class Application
{
    ...
    private function createQuestPage(array $values = [], ?string $error = null): void
    {
        ...
        $errorHtml = $error ? '<div class="notice error" role="alert">' . self::escape($error) . '</div>' : '';
        $body = '<section class="form-panel"><p class="eyebrow">New Quest</p><h1>What kind of undertaking is this?</h1><p>A simple action stays simple. Projects and journeys can grow steps and milestones after saving.</p>' . $errorHtml . '<form method="post" action="/quests">' . $this->csrfField() .
    ...
    {
        $this->requireCsrf(); $account = Security::requireAccount();
        $values = ['quest_type'=>(string)($_POST['quest_type']??'action'),'title'=>(string)($_POST['title']??''),'description'=>(string)($_POST['description']??''),'pillar_key'=>(string)($_POST['pillar_key']??''),'frequency'=>(string)($_POST['frequency']??'none'),'interval_count'=>(string)($_POST['interval_count']??'1'),'starts_on'=>(string)($_POST['starts_on']??date('Y-m-d')),'ends_on'=>(string)($_POST['ends_on']??''),'weekdays'=>(array)($_POST['weekdays']??[]),'monthly_mode'=>(string)($_POST['monthly_mode']??'day_of_month'),'day_of_month'=>(string)($_POST['day_of_month']??date('j')),'ordinal_week'=>(string)($_POST['ordinal_week']??'1'),'ordinal_weekday'=>(string)($_POST['ordinal_weekday']??'1'),'timezone'=>(string)($_POST['timezone']??'America/New_York')];
