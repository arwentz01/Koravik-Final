...
try {
    ...
    foreach ([
        'quest.completed' => 'Allows Epic Ordinary to receive a minimized fact when a Quest occurrence is completed. Quest notes and full private records are not shared.',
        'player.returned' => 'Allows Epic Ordinary to receive a minimized fact that the player returned after an extended absence. No Quest details are included.',
    ] as $factKey => $explanation) {
        $pdo->prepare('INSERT INTO world_fact_permissions (installation_id, fact_key, granted, explanation, granted_at, revoked_at, updated_at) VALUES (:installation_id, :fact_key, 1, :explanation, :granted_at, NULL, :updated_at) ON DUPLICATE KEY UPDATE granted = 1, explanation = VALUES(explanation), granted_at = COALESCE(world_fact_permissions.granted_at, VALUES(granted_at)), revoked_at = NULL, updated_at = VALUES(updated_at)')
            ->execute(['installation_id'=>$installationId,'fact_key'=>$factKey,'explanation'=>$explanation,'granted_at'=>$now,'updated_at'=>$now]);
    }
    ...
    $pdo->commit();
    echo "Seeded Build 008 account, Quest occurrence, World installation, and default fact permissions.\n";
